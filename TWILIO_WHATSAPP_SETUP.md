...
```
POST /whatsapp-webhook?secret=YOUR_SECRET
...
3. Set **"When a message comes in"** to:
   ```
   POST https://YOUR_PROJECT.functions.supabase.co/whatsapp-webhook?secret=YOUR_SECRET
   ```
...
- **Shared Secret**: Always include `?secret=YOUR_SECRET` in the webhook URL
